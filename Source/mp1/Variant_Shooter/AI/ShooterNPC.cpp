...
void AShooterNPC::PostInitializeComponents()
{
...
void AShooterNPC::BeginPlay()
{
	...
	if(HasAuthority())
	{
		...
		Weapon = GetWorld()->SpawnActor<AShootWeapon>(WeaponClass, GetActorTransform(), SpawnParams);
		if (Shoot)Shoot->EquipWeapon(Weapon);
	}
}
...
void AShooterNPC::AttachShootWeaponMeshes(AShootWeapon* WeaponToEquip)
{
	...
	// attach the weapon meshes
	WeaponToEquip->GetFirstPersonMesh()->AttachToComponent(GetFirstPersonMesh(), AttachmentRule, FirstPersonWeaponSocket);
	WeaponToEquip->GetThirdPersonMesh()->AttachToComponent(GetMesh(), AttachmentRule, FirstPersonWeaponSocket);
...
void AShooterNPC::DoFireAction(AActor* ActorToShoot)
{
	CurrentAimTarget = ActorToShoot;
...
void AShooterNPC::DoStopFireAction()
{
	bIsShooting = false;
	if (Shoot)
	{
